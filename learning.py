...
class Learning:
    ...
    def adapt_city(self, agent):
        ...
        current_state = CityState(
            ...
            agent.city.get('infrastructure_development_rate', 0)  # Assuming infrastructure development rate may not be initialized
        )
        ...
        next_state = CityState(
            agent.city['population_growth_rate'], 
            agent.city['environmental_impact_rate'], 
            agent.city.get('infrastructure_development_rate', 0)  # Include infrastructure in next state
        )
